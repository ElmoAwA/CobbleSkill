...
public class RandomLocationsNearOriginTargeter extends AbstractTargeter {
    ...
    @SerializedName(value = "onsurface", alternate = {"onsurf", "os"})
    ...
    public List<Target> find(SkillTree tree) {
        ...
        for (int i = 0; i < amt; i++) {
            for (int attempt = 0; attempt < attemptsPerPoint; attempt++) {
                ...
                double r = Math.sqrt(u) * (maxR - minR) + minR;
                ...
                Vec3 candidate = Util.safeSpawnPosition(level, new Vec3(x, y, z), maxR, 0, 1, true, placeOnSurface);
